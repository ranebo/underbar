(function() {
  ...
  /*
  THIS WAS MY FIRST SOLVE: Then noticed I could use _.each.
  ...
  */
  ...
  _.zip = function() {
    var result = [];
    var args = [].slice.call(arguments);
    // must be better way to find longest array... if the first array is always the longest array, longest could be replaced with args[0].length
    var longest = _.reduce(args, function(total, current){
      return total > current.length ? total : current.length;
    }, 0);
  ...
  _.intersection = function() {
    var result = [];
    var flatArgs = _.flatten([].slice.call(arguments));

    for (var i = 0; i < flatArgs.length; i++){
      var target = flatArgs.pop();

      if (_.contains(flatArgs, target) && !_.contains(result, target)){
        result.push(target);
      }
    }
